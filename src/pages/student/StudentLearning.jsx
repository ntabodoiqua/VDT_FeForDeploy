...
const StudentLearning = () => {
  ...
  const initializeTimer = (quiz, attemptStartTime) => {
    ...
    const startTime = attemptStartTime
      ? new Date(attemptStartTime)
      : new Date();
  ...
  const fetchEnrollmentAndProgress = async () => {
    try {
      ...
      if (enrollmentResponse.code === 1000 && enrollmentResponse.result) {
        ...
        const progressResponse = await fetchEnrollmentProgressApi(
          enrollmentResponse.result.id
        );
    ...
    try {
      ...
      if (lessonResponse.code === 1000 && lessonResponse.result) {
        ...
        if (quiz) {
          ...
          console.log(
            "Quiz attached to lesson during initial load:",
            quiz.title
          );
        ...
        if (enrollment?.id) {
          ...
          const hasQuiz =
            getQuizForLessonFromData(currentLessonData.id, quizzesData) !==
            null;
  ...
  const handleStartQuiz = async () => {
    ...
    try {
      ...
      try {
        ...
        if (
          currentAttemptResponse.code === 1000 &&
          currentAttemptResponse.result
        ) {
          ...
          if (
            attemptData.attemptAnswers &&
            Array.isArray(attemptData.attemptAnswers)
          ) {
            attemptData.attemptAnswers.forEach((attemptAnswer) => {
              if (attemptAnswer.selectedAnswer) {
                existingAnswers[attemptAnswer.question.id] =
                  attemptAnswer.selectedAnswer.id;
              }
            });
          }
      ...
      if (
        ...
        new Date() < new Date(quizDetails.startTime)
      ) {
      ...
      // Tạo attempt mới
      const startResponse = await startQuizAttemptApi(
        currentLesson.quiz.id,
        courseId
      );
  ...
  const handleAnswerQuestion = async (questionId, answerId) => {
    ...
    if (quizAttempt?.id) {
      try {
        const response = await answerQuestionApi(
          quizAttempt.id,
          questionId,
          answerId
        );
        if (response.code === 1000 && response.result) {
          ...
          console.log(
            `Points earned: ${answerData.pointsEarned}/${answerData.question.points}`
          );
    ...
    try {
      ...
      if (submitResponse.code === 1000 && submitResponse.result) {
        ...
        setQuizResult({
          ...
          totalPossibleScore:
            result.totalQuestions > 0
              ...
              : result.score, // Calculate from percentage
        ...
        if (result.isPassed && currentLesson?.id) {
          console.log(
            "Updating quiz completion status for lesson:",
            currentLesson.id
          );
        ...
        const resultMessage = result.isPassed
          ...
          : `Bạn đạt ${result.percentage?.toFixed(1)}% (cần ${
              result.passingScore
            }% để đạt)`;
  ...
  const loadQuizHistory = async (quizId, lessonId = null) => {
    if (!quizId || !courseId) {
      console.warn("loadQuizHistory: Missing quizId or courseId", {
        quizId,
        courseId,
      });
      return;
  ...
  const loadAllQuizCompletionStatus = async () => {
    ...
    console.log(
      "Loading quiz completion status for all lessons in course:",
      courseId
    );
    ...
    const completionPromises = lessons.map(async (lesson) => {
      ...
      if (quiz) {
        try {
          console.log(
            `Loading best score for lesson ${lessonId}, quiz ${quiz.id} in course ${courseId}`
          );
          const bestScoreResponse = await getBestQuizScoreApi(
            quiz.id,
            courseId
          );
          if (bestScoreResponse?.code === 1000 && bestScoreResponse.result) {
            console.log(
              `Best score found for lesson ${lessonId}:`,
              bestScoreResponse.result
            );
            return {
              lessonId,
          ...
          console.error(
            `Error loading best score for lesson ${lessonId}:`,
            error
          );
        }
      }
  ...
  const getCurrentQuestion = () => {
    ...
    if (currentQuiz.questions && Array.isArray(currentQuiz.questions)) {
      ...
      console.log(
        "Using questions from currentQuiz.questions:",
        questions.length
      );
    }
    // Fallback: try to get from quizAttempt.attemptAnswers (for student view)
      ...
      console.log(
        "Using questions from quizAttempt.attemptAnswers:",
        questions.length
      );
    }
    ...
    if (currentQuestionIndex < 0 || currentQuestionIndex >= questions.length) {
      console.warn(
        `Invalid question index: ${currentQuestionIndex}, total questions: ${questions.length}`
      );
      return null;
    }
  ...
  const getAnsweredQuestionsCount = () => {
    ...
    return Object.keys(quizAnswers).filter(
      (questionId) => quizAnswers[questionId] != null
    ).length;
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (quizVisible && !quizResult) {
        e.preventDefault();
        e.returnValue =
          "Bạn đang làm quiz. Nếu rời khỏi trang, bài làm sẽ bị mất!";
    ...
    <div style={{ height: "100%", overflow: "auto" }}>
      ...
      <List
        ...
        renderItem={(lesson, index) => {
          ...
          if (index === 0 && hasQuiz) {
            console.log(
              "Quiz detected for first lesson:",
              getQuizForLesson(lessonId).title
            );
          }
          ...
          return (
            <List.Item
              style={{
                ...
                cursor: "pointer",
                backgroundColor:
                  index === currentLessonIndex ? "#e6f7ff" : "transparent",
                borderLeft:
                ...
                title={
                  <div>
                    <Text
                      ...
                      style={{
                        color:
                          index === currentLessonIndex ? "#1890ff" : "#333",
                        fontSize: "14px",
                      }}
                    >
                      {lesson.lesson?.title ||
                        ...
                        <Card
                          ...
                          style={{
                            ...
                            borderLeft: `4px solid ${
                              bestScore?.isPassed ? "#52c41a" : "#1890ff"
                            }`,
                            backgroundColor: bestScore?.isPassed
                              ? "#f6ffed"
                              : undefined,
                          }}
                              ...
                              >
                                <Space>
                                  ...
                                  <Text strong style={{ color: "#52c41a" }}>
                                    Điểm cao nhất: {bestScore.score?.toFixed(1)}{" "}
                            ...
                            <div>
                              <Space wrap size="small">
                                <Tag color="blue" size="small">
                                  {currentLesson.quiz.totalQuestions ||
                          ...
                          <List
                            ...
                            renderItem={(doc) => (
                              <List.Item
                                ...
                                onClick={() =>
                                  handleDocumentPreview(doc, false)
                                }
                                actions={[
                                  <Button
                                    type="text"
            ...
            onClick={() => {
              if (previewDocument) {
                handleDocumentDownload(
                  previewDocument,
                  lessonDocuments.includes(previewDocument)
                );
              }
            }}
          >
              ...
              return (
                ...
                >
                  <img
                    src={previewUrl}
                    alt={
                      previewDocument.title || previewDocument.originalFileName
                    }
                    style={{
                      maxWidth: "100%",
                      maxHeight: "100%",
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              ...
              <Button
                ...
                title={
                  !quizResult && timerActive
                    ? "Không thể đóng quiz khi đang làm bài"
                    : "Đóng quiz"
                }
              />
            </div>
        ...
        {quizLoading ? (
              ...
              message={
                <div style={{ textAlign: "center" }}>
                  <Title
                    ...
                    style={{
                      margin: "8px 0",
                      color: quizResult.isPassed ? "#52c41a" : "#ff4d4f",
                    }}
                  >
                    {quizResult.isPassed ? (
                      <>
              ...
              <Col span={6}>
                <Card>
                  <Statistic
                    ...
                    valueStyle={{
                      color:
                        quizResult.percentage >= quizResult.passingScore
                          ? "#52c41a"
                          : "#ff4d4f",
                    }}
                    suffix="%"
          ...
          currentQuiz && (
            ...
            <div>
                ...
                >
                  ...
                  <Space>
                    ...
                    <Button
                      size="small"
                      disabled={
                        currentQuestionIndex === getTotalQuestionsCount() - 1
                      }
                      onClick={() =>
                        setCurrentQuestionIndex((prev) => prev + 1)
                      }
              ...
              {getCurrentQuestion() ? (
                <Card
                  title={`Câu ${currentQuestionIndex + 1}`}
                  style={{ marginBottom: "24px" }}
                >
                  ...
                  {currentQuiz.questions?.map((question, index) => {
                    ...
                    return (
                      <Button
                        ...
                        style={{
                          backgroundColor:
                            isAnswered && !isCurrent ? "#52c41a" : undefined,
                          borderColor:
                            isAnswered && !isCurrent ? "#52c41a" : undefined,
              ...
              <div style={{ textAlign: "center" }}>
                <Space size="large">
                  <Button
                    ...
                    onClick={() => {
                      Modal.confirm({
                        ...
                        content:
                          "Bạn có chắc chắn muốn hủy quiz? Tất cả dữ liệu đã làm sẽ được lưu lại.",
                        okText: "Hủy quiz",
                  ...
                  >
                    Nộp bài ({getAnsweredQuestionsCount()} /{" "}
                    {getTotalQuestionsCount()})
                  </Button>
                </Space>
              </div>
                ...
                <Row gutter={16} align="middle">
                  ...
                  <Col flex="auto">
                    ...
                    <Text type="secondary" style={{ fontSize: "12px" }}>
                      Lần {bestScore.attemptNumber} •{" "}
                      {dayjs(bestScore.completedAt).format("DD/MM/YYYY HH:mm")}
                    </Text>
                  </Col>
                </Row>
              </Card>
                          ...
                          <div>
                            ...
                            <div>
                              <Text
                                ...
                                style={{ fontSize: "12px" }}
                              >
                                {dayjs(attempt.completedAt).format(
                                  "DD/MM/YYYY"
                                )}
                              </Text>
                            </div>
                          ...
                          <div style={{ textAlign: "center" }}>
                            <div
                              style={{
                                fontSize: "20px",
                                fontWeight: "bold",
                                color: attempt.isPassed ? "#52c41a" : "#fa8c16",
                              }}
                            >
                      ...
                      <Col span={6}>
                        <Statistic
                          ...
                          prefix={
                            quizHistory.length > 1 &&
                            (quizHistory[0].percentage || 0) -
                              (quizHistory[quizHistory.length - 1].percentage ||
                                0) >
                              0
                              ? "+"
                              : ""
